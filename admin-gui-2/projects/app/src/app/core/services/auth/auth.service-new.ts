...
import {BehaviorSubject, concatMap, mapTo, Observable, of, switchMap, throwError} from 'rxjs';
...
export class AuthService
{
  ...
  public changeCompany(companyId: string): Observable<SpvitaminSecurity.AuthorizationToken>
  {
    return this.tokenService.changeCompany(companyId);
  }


  public updateRoles(): Observable<any>
  {
    this.isRefreshing = true;
    const token = this.getToken();
    return this.logout(false).pipe(
      concatMap(() => this.tokenService.updateRoles(token)),
      concatMap((updatedToken: SpvitaminSecurity.AuthorizationToken) =>
      {
        return this.renewToken$(updatedToken);
      }),
      tap((authToken: SpvitaminSecurity.AuthorizationToken) =>
      {
        this.loginSuccess(authToken, false);
      }),
      finalize(() => this.isRefreshing = false),
      catchError(error =>
      {
