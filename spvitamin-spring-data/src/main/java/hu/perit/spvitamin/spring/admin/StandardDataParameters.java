...
class StandardDataParameters
{
    ...
    public ServerParameterList getParameterList()
    {
        ...
        {
            params.add(getGroupName(entry.getKey()), ServerParameterListBuilder.of(entry.getValue()));
        }
    ...
    {
        return MessageFormat.format("{0}: {1}", DatasourceProperties.class.getSimpleName(), name);
    }
}
